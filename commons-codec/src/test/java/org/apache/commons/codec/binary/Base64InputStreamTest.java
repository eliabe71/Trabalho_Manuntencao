...
public class Base64InputStreamTest {
    ...
    @Test
    public void testAvailable0() throws Throwable {
        final InputStream ins = new ByteArrayInputStream(StringUtils.getBytesIso8859_1(ENCODED_B64));
        try (final Base64InputStream b64stream = new Base64InputStream(ins)) {
            assertEquals(0, b64stream.available());
        }
    ...
    private void testByReadEOF(final byte[] encoded, final byte[] decoded, final int chunkSize, final byte[] separator) throws Exception {
    ...
    private void testByReadStill(final byte[] encoded, final byte[] decoded, final int chunkSize, final byte[] separator) throws Exception {
    ...
    private void testByteByByte(final byte[] encoded, final byte[] decoded, final int chunkSize, final byte[] separator) throws Exception {
        ...
        private void testByteByByteEOF(final byte[] encoded, final byte[] decoded, final int chunkSize, final byte[] separator) throws Exception {
